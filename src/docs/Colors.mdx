...
export const Swatch = ({ name, varName }) => {
  const [oklch, setOklch] = React.useState("")
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        <div style={{ fontWeight: 600, color: "var(--foreground)" }}>
          {name} <span style={{ fontWeight: 400, color: "var(--muted-foreground)", fontFamily: "monospace", fontSize: 12 }}>({oklch})</span>
        </div>
...
Tokens disponíveis para uso com `recharts` (ou qualquer outra lib de gráficos) — a lib **não exporta** um componente de chart, apenas os tokens.
